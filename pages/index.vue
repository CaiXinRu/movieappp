<template>
  <div>
    ...
    <!-- <template v-if="loading"> -->
      <LoadingPage v-if="$fetchState.pending"/>
    <!-- </template> -->
  ...
  methods: {
    async getMovies() {
      ...
      const data = axios.get(
        'https://api.themoviedb.org/3/movie/now_playing?api_key=bc38f964cf7dfad975028d41d88d807f&language=en-US&page=1'
      )
    ...
    async searchMovies () {
      // this.loading = true;
      const data = axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=bc38f964cf7dfad975028d41d88d807f&language=en-US&page=1&query=${this.searchInput}`
        )
        const result = await data
        result.data.results.forEach((movie) => {
